...
{
	string tmpdir_basename = TempDirBaseName();
	char* temp = new char[ tmpdir_basename.length() + 1 ];
	strncpy( temp, tmpdir_basename.c_str(), tmpdir_basename.length() + 1 );
	char* tmpdirname = tempnam( local_db_path.c_str(), TempDirBaseName().c_str());
	int success = mkdir(tmpdirname, 0777);
	if ( success != 0){
...
#ifndef WIN32
	char* dir_name = tempnam( local_db_path, TempDirBaseName() );
	int s = mkdir(dir_name, 0777);
	if ( s != 0 ){
		perror( "Error creating temp directory" );
		throw dir_name;
	}
	char* success = dir_name;
#else
