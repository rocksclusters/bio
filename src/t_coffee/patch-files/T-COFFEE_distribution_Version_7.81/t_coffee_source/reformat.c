...
/*END INITIALIZATION*/
 	...
 	if ( argc==1 || strm6 ( argv[1], "h", "-h", "help", "-help", "-man", "?"))
		...
		fprintf ( stdout, "\n     +treelist2splits......List all the splits in a list of files");
...
int main_output  (Sequence_data_struc *D1, Sequence_data_struc *D2, Sequence_data_struc *DST, char *out_format, char *out_file)
	...
	else if (strm5 ( out_format, "strict_clustal","clustal_aln", "clustalw","clustal", "clustalw_aln"))
	  {		 
	    if (!D1)return 1;
...
{
  output_generic_clustal_aln (name, B, "tc_clustal");
}
void output_strict_clustal_aln ( char *name, Alignment *B)
{
  output_generic_clustal_aln (name, B, "strict_clustal");
}
...
void modify_data  (Sequence_data_struc *D1in, Sequence_data_struc *D2in, Sequence_data_struc *DSTin, char **action_list,int n_actions, Action_data_struc *RAD)
	 ...
	 {
	   fprintf ( stdout, "SP Score: %.2f", sum_pair (DST->A,ACTION(1),atoi(ACTION2(2,"0")),atoi(ACTION2(3,"0"))));
	 }
       ...
       else if ( strm (action, "remove_seq") || strm (action, "rm_seq"))	 
	 {
       ...
       else if (  strm (action, "aln2overaln"))
	 {
	   ...
	   if (D2->A)D1->A=aln2clean_pw_aln (D1->A, ACTION(1),t, f,p1,p2,p3, "fsa2");
	   else D2->A=aln2clean_pw_aln (D1->A, ACTION(1),t, f,p1,p2,p3, "fsa1");
	   D1->S=aln2seq ( D1->A);
	 }
       else if (  strm (action,"aln2unalign"))
	 {
	   ...
	   if (!D2 || !D2->A)
	     {
	       SA=aln2clean_pw_aln (D1->A,"number",0,0,0,0,0, NULL);
	     }
	   else
	     {
	       SS=aln2seq(SA);
	   ...
	   if ( clean_flag==1)
	     {
	       ...
	       DST->A=aln2clean_pw_aln (D1->A,"number",0,0,0,0,0, NULL);
	     }
...
//Special modes
       ...
       else if ( strm ( action, "overaln"))//Evaluate the capacity to predict over-aligned regions
	 {
	   ...
	   D2->A=aln2clean_pw_aln (D2->A, "lower",ATOI_ACTION(1), ATOI_ACTION(2), ATOI_ACTION(3), ATOI_ACTION(4), ATOI_ACTION(5), NULL);
